<%-- 
    Document   : Formulário de Exclusão de Cliente
...
<%
    ...
    if (idParam != null && !idParam.isEmpty()) {
        try {
            ...
            cliente = cliDAO.consultar_id(cli);

            if (cliente == null) {
                errorMessage = "Cliente com ID " + clienteId + " não encontrado.";
            }
        } catch (NumberFormatException e) {
            errorMessage = "ID do cliente inválido.";
        } catch (Exception e) {
            errorMessage = "Erro ao consultar cliente: " + e.getMessage();
        }
    }
%>
...
<body>
    <div class="container">
        ...
        <form method="get" action="cli_excluir_form.jsp">
            <label for="id">Digite o ID do Cliente:</label>
            <input type="number" id="id" name="id" required value="<%= idParam != null ? idParam : "" %>">
            <button type="submit" class="button" style="background-color: #007bff; width: 100%;">Buscar Cliente</button>
        </form>
        ...
        <% if (cliente != null) { %>
            <div class="data-details">
                <h3>Dados do Cliente a ser Excluído:</h3>
            ...
            <div class="confirmation">
                ...
                <div class="confirmation-buttons">
                    <form method="post" action="cli_exclui_action.jsp" style="display: inline;">
                        <input type="hidden" name="id" value="<%= cliente.getId() %>">
                        <button type="submit" class="button btn-danger">Sim, Excluir</button>
                    </form>
                    <a href="cli_excluir_form.jsp" class="button btn-secondary">Não, Cancelar</a>
                </div>
            </div>
        <% } %>
